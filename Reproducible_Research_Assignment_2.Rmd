# Most Harmfull Events Across United States From 1950 To 2011
...
download.file(fileUrl, destfile = "./NOAA_Storm_Database/storm_data.csv.bz2", method = "curl")
...
```{r harmful_to_health_1, cache = TRUE}
library(dplyr)
library(lubridate)
library(ggplot2)
library(scales)
...
We can now move on to see the picture of these 4 intervals taken togetheer.  

```{r harmful_to_health_2, cache = TRUE}
# Single Plot
...
```{r economic_consequences, cache = TRUE}
...
```{r economic_consequences_processing_1, cache = TRUE}
# 1
# 1_a
...
``` {r economic_consequences_processing_2, cache = TRUE}
# 2
# 2_a
